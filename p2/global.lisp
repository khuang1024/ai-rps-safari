<<<<<<< HEAD
;;; 1. legalp is modified 
;;; 2. random-gent is added to make the demo run
=======
;;; legalp is deleted
>>>>>>> 64124b9affa98e3b0e7456e66182f52beb514804
...
(defun monitor (agent-list numtimes)
  (let (results (agents (make-hash-table :size 100)))
    (dolist (agnt agent-list)
      (setf (gethash agnt agents) 0)) ; initialize all these agents and put their ranks 0
    (dotimes (x numtimes) ; run each 1000-round tournaments numtimes times
      (let ((curRank 0) (results (tournament agent-list 1000))) ; initialize curRank = 0, results = list returned by tournament
